...
// 修复数据目录权限（解决 volume 挂载后权限不足问题）
function fixDataDirPermissions() {
  const dataDir = process.env.DATA_DIR || '/app/data';
  const subdirs = ['playrecords', 'favorites', 'searchhistory', 'skipconfigs'];
  ...
  try {
    ...
    console.log(`✅ 数据目录已就绪: ${dataDir}`);
  } catch (err) {
    console.warn(`⚠️ 数据目录创建失败: ${err.message}，将使用临时目录`);
  }
}

fixDataDirPermissions();

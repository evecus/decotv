...
function LivePageClient() {
  ...
  const attemptAutoFailover = async (reason: string) => {
    ...
    try {
      ...
      const rankedCandidates = [...candidates].sort((a, b) => {
        const healthA = (channelHealthMapRef.current[a.id]?.status || 'unknown') as ChannelHealthStatus;
        const healthB = (channelHealthMapRef.current[b.id]?.status || 'unknown') as ChannelHealthStatus;
  ...
  class CustomHlsJsLoader extends Hls.DefaultConfig.loader {
    constructor(config: any) {
      ...
      const load = self.load.bind(self);
      self.load = function (context: any, config: any, callbacks: any) {
        // 所有的请求都带一个 source 参数
        try {
